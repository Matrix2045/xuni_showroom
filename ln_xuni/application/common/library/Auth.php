...
class Auth
{
    ...
    {
        // 检测个人/团队名称或手机号是否存在
        if($param['mobile'] && User::getByMobile($param['mobile'])) {
        ...
        $params['invite_code'] = Random::alnum(8);
        ...
        try {
            ...
            //注册成功的事件
            Hook::listen("user_register_successed", $this->_user, $params);
            Db::commit();
        } catch (Exception $e) {
     ...
     * @return boolean
     */
    public function thirdRegister($nickname)
    {
        $ip = request()->ip();
        $time = time();
        $params = [
        ...
        $params['password'] = $this->getEncryptPassword('123456', $params['salt']);
        $params['invite_code'] = Random::alnum(8);
    ...
    public function login($mobile, $password)
    {
        ...
        if (!$user) {
            $this->setError('Mobile is incorrect');
            return false;
        }

        if ($user->status != 'normal') {
            $this->setError('Mobile is locked');
            return false;
        }
